...
protected:
    ...
    * Init */
    ...
    void initParameter(const uint32_t index, Parameter& param) override
    {
        switch (index)
        {
        {% for p in active %}case kParameter_{{cid(p.meta.symbol|default("" ~ loop.index))}}:
            ...
            break;
        {% endfor %}
        {% for p in passive %}case kParameter_{{cid(p.meta.symbol|default("" ~ (active|length+loop.index)))}}:
    ...
    * Internal data */
    ...
    float getParameterValue(const uint32_t index) const override
    {
        switch (index)
        {
        {% for p in active + passive %}case kParameter_{{cid(p.meta.symbol|default("" ~ loop.index))}}:
            return dsp->{{p.var}};
        {% endfor %}
        default:
            return 0.0f;
